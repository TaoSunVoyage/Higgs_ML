...
# def add_eta(x):
#     """Add new eta features."""
#     # PRI_lep_eta - PRI_tau_eta
#     r1 = delta_angle_norm(x[:,17], x[:,14]).reshape(-1, 1)
#     # PRI_jet_leading_eta - PRI_tau_eta
#     r2 = delta_angle_norm(x[:,24], x[:,14]).reshape(-1, 1)
#     # PRI_jet_subleading_eta - PRI_tau_eta
#     r3 = delta_angle_norm(x[:,27], x[:,14]).reshape(-1, 1)
#
#     x = np.concatenate([x, r1, r2, r3], axis=1)
#
#     return x
#
#
